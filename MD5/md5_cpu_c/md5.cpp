...
//==============================================================================================  
    ...
    index = (unsigned int)((context->count[0] >> 3) & 0x3F);	//index有实质变化在MD5Final中的第一个MD5Update调用,计算出了待加密数据模64字节的余值
  
    /* Update number of bits *//*更新已有信息的bits长度*/  
	//原长度信息是字节，现在左移三位用位来表示原始信息的长度信息
    if((context->count[0] += ((UINT4)inputLen << 3)) < ((UINT4)inputLen << 3))  
        context->count[1]++;  //这句何时才会执行到？
...
//===============================================================================================
    ...
    */  
	...
	//在MD5Final中，如果待加密长度在56到64字节之间，才会进入这个if，分为两种情况：
	//1、第一次Final调用Update时，可能进入这个if，按照算法约定，补齐这64位，并进行一次摘要计算
	//2、第二次Final调用Update时，肯定进入这个if,此时则加上了待加密字符串的长度信息，进行最后一次摘要计算，大功告成
    if(inputLen >= partLen) {  
        ...
        */  
		//MD5Final调用的Update无论如何也不可能进入这个循环了，但是注意这里对i进行了赋值
	...
	//在Md5Final第一次调用Update中，将待加密数据补全至理想位数，分为两种情况
	//1:在数据长度模64后值在56-64之间的，此时已经新开了一段64位的区间，之前区间都做了MD5摘要，这个新区间补全至了第56位
	//2:在数据长度模64后值小于56的，将这段数据补齐至第56位
...
{  
    ...
    /* Save number of bits */  
    /*将要被转换的信息(所有的)的bits长度拷贝到bits中，原本是用int表示，现在是用char表示*/  
    ...
    padLen = (index < 56) ? (56 - index) : (120 - index);		//56+64=120,即如果大于56位，则新开一个64位的分组，从本处一直填充到下64位的第56位
